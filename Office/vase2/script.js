...
window.onload = (event) => {
  ...
  let vaseRotation = window.sessionStorage.getItem(`vase2Rotation`);
  let vase = document.getElementById('vase');
  if(vaseRotation) {
    vase.src = `./images/1_a${vaseRotation}.webp`;
...
function rotateVase(clockwise) {
  ...
  vase.src = `./images/1_a${rotationNumber}.webp`;
  vase.rotation = rotationNumber;
  console.log(rotationNumber);
  window.sessionStorage.setItem(`vase2Rotation`, rotationNumber);
  setTimeout(() => {
