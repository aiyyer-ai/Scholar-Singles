...
function setScreen() {
      ...
      let gameArea = document.getElementById(`allGame`);
      document.body.style.backgroundSize = `100% ${parseInt(((gameArea.clientHeight + gameArea.offsetTop + 30) / screenKeeper.clientHeight) * 100 * scale) / 100 * 1080}px`;
}
...
function dragElement(elmnt) {
      ...
      function closeDragElement(e) {
            ...
            if (inventoryItem) {
                  ...
                  let elmntCenter = parseInt(elmnt.offsetLeft + (elmnt.clientWidth / scale) / 2);
                  let centerToOrigin = {};
                  let snapCenters = snapToWidth.map((snapPoint) => {
                        return parseInt(snapPoint + (elmnt.clientWidth / scale) / 2);
                  });
                  for (i in snapCenters) {
                        centerToOrigin[snapCenters[i]] = snapToWidth[i];
                  }
                  let placeLocation = centerToOrigin[closest(elmntCenter, snapCenters)];
...
function startGame(permutationArray, startData, startOrder) {
      ...
      Array.from(indicatorLights).forEach(function (singleIndicator) {
            ...
            singleIndicator.style.width = `120px`;
            Array.from(allCheckboxes).forEach(function (singleInput) {
                  ...
                  indicatorLight.style.width = `60px`;
                  indicatorLight.style.height = `60px`;
