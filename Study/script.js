...
window.onload = (event) => {
      ...
      let mirrorClosed = window.sessionStorage.getItem(`mirrorClosed`);
      let mirrorBlocker = document.getElementById(`mirrorBlocker`);
      if (mirrorClosed) {
            mirrorBlocker.classList.add(`notransition`);
            mirrorBlocker.offsetHeight;
      } else {
            window.sessionStorage.setItem(`mirrorClosed`, `true`);
      }
      mirrorBlocker.style.transform = `none`;

      let fuseBoxData = window.sessionStorage.getItem(`foyerFuseBox`);
      if (fuseBoxData) {
            let doorFoyerDiv = document.getElementById(`doorFoyer`);
            doorFoyerDiv.children[0].src = `./images/doorFoyerOpen.webp`;
            doorFoyerDiv.style.height = 585 + "px";
            doorFoyerDiv.style.bottom = 50 + "px";
            doorFoyerDiv.style.left = 185 + "px";
      }
      let workshopAccess = window.sessionStorage.getItem(`workshopAccess`);
      if (workshopAccess) {
            let wallDiv = document.getElementById(`wallDoor`);
            wallDiv.openAlready = true;
            wallDiv.style.transition = `none`;
            wallDiv.style.transform = `translate3d(100px, 300px, 350px)`;
      }

      let podiumPull = JSON.parse(window.sessionStorage.getItem(`podiumPull`));
      if(podiumPull) {
            openWall();
      }
}
...
function pullCheck(div, event) {
      var pos1 = 0, pos2 = 0, pos3 = 0, pos4 = 0;
      pos3 = event.clientX;
      pos4 = event.clientY;
      document.onmousemove = openCheck;
      document.onmouseup = leaveCheck;
      function openCheck(e) {
            let inventory = JSON.parse(window.sessionStorage.getItem(`inventory`));
            pos1 = pos3 - e.clientX;
            pos2 = pos4 - e.clientY;
            if (pos2 > 50 && inventory.scholarDiary) {
                  document.onmousemove = null;
                  document.onmouseup = null;
                  openWall();
                  setTimeSpent();
                  let finalTime = JSON.parse(window.sessionStorage.getItem(`timeData`));
      ...
      function leaveCheck(e) {
            ...
            pos2 = pos4 - e.clientY;
            if (pos2 < 50) {
                  window.location.href = `./podium/index.html`;
            }
      }
}

function rotateLamp(lampIMG) {
      if (!lampIMG.rotation) {
            lampIMG.rotation = 0;
      }
      if (lampIMG.rotation == 0) {
            lampIMG.rotation = 90;
      } else {
            lampIMG.rotation = 0;
            let fuseBoxLights = document.getElementById(`fuseBoxLights`);
            fuseBoxLights.style.visibility = ``;
            window.sessionStorage.setItem(`fuseBoxLights`, `false`);
      }
      lampIMG.style.transform = `rotate(${lampIMG.rotation}deg)`;
      ...
      if (arraysEqual(Object.values(lecternRotations), [`six`, `three`, `four`, `one`]) && lampIMG.rotation == 90) {
            let fuseBoxLights = document.getElementById(`fuseBoxLights`);
            fuseBoxLights.style.visibility = `visible`;
            window.sessionStorage.setItem(`fuseBoxLights`, `true`);
      }
}

function openWall() {
      let wallDiv = document.getElementById(`wallDoor`);
      if(!wallDiv.openAlready) {
            window.sessionStorage.setItem(`workshopAccess`, `true`);
            wallDiv.openAlready = true;
            setTimeout(() => {
                  wallDiv.style.transform = `translate3d(100px, 300px, 350px)`;
                  setTimeout(() => {
